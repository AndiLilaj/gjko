...
func TestNextToken(t *testing.T) {
	...
	tests := []struct {
		...
		{token.INT, "5"}, {token.SEMICOLON, ";"}, {token.IDENT, "ten"},
		{token.ASSIGN, "="}, {token.INT, "10"}, {token.SEMICOLON, ";"},
		{token.LET, "let"}, {token.IDENT, "add"}, {token.ASSIGN, "="},
		{token.LET, "let"}, {token.FUNCTION, "fn"}, {token.LPAREN, "("},
